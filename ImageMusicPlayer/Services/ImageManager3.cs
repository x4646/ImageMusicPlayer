...
{
    public class ImageManager3 : IImageManager
    {
        ...
        private async Task<Image?> LoadImageAsync(int index)
        {
            ...
            Image? image = await Task.Run(() => LoadImageFromFileAsync(filePath));
            if (image == null)
            {
        ...
        private static async Task<Image?> LoadImageFromFileAsync(string filePath)
        {
            try
            {
                // 异步读取文件数据，避免同步 I/O 占用线程
                byte[] imageData = await File.ReadAllBytesAsync(filePath);

                using (MemoryStream ms = new MemoryStream(imageData))
                {
                    // 这里同步加载图片（GDI+ 不支持异步加载）
                    using (Image originalImage = Image.FromStream(ms, useEmbeddedColorManagement: false, validateImageData: false))
                    {
                        int maxDimension = 1920; // 可根据需求调整，超过此尺寸则进行缩放
        ...
        /// </summary>
        private static Image? LoadImageFromFile1(string filePath)
        {
            try
            {
        ...
        private async Task PreloadAdjacentImagesAsync(int currentIndex)
        {
            ...
            foreach (var i in indices)
            {
                ...
                var img = await Task.Run(() => LoadImageFromFileAsync(imagePaths[i]));
                if (img == null)
                {
